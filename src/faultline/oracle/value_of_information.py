...
from faultline.oracle.model import DiagnosticProblem
...
def analyze_action(problem: DiagnosticProblem, action: Action) -> ActionValue:
    """Evaluate one diagnostic action followed by the best contingent commitment."""
    branches = root_branches(problem)
    ...
    information_gain = entropy_bits(problem.prior) - expected_posterior_entropy
    return ActionValue(
        action=action,
